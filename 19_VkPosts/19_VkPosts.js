...
function getPosts() {
  if (!isLoading) { // Проверяем, не выполняется ли запрос уже
    isLoading = true;
    //Создаем скрипт
    ...
    const v = 5.131;
    //Ключ доступа
    const access_token = "7e25ee9a7e25ee9a7e25ee9a377d30638e77e257e25ee9a1add22eaf3c7377b714aae49";
 ...
 data.items.map((item) => {
    const postElement = `<li class="post-list__item">
          ...
          <div class="post-list__item__subText">
            <div class="post-list__item__date">${new Date(item.date * 1000).toLocaleString("ru", options)}</div>
